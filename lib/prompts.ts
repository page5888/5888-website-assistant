...
- \`<html lang="zh-Hant-TW">\`（現代標準,不要用 zh-TW）
- **必須載入 Google Fonts**（<head> 放 preconnect + <link>）:
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif+TC:wght@700;900&family=Noto+Sans+TC:wght@400;700&family=Playfair+Display:wght@700;900&display=swap" rel="stylesheet">
  （每個字型最多 2 字重,一律帶 display=swap）
...
1. **<title> 規則**：
   ...
   - 長度 20-32 中文字
   - 地區 + 核心服務放最前面（搜尋意圖）
   - 差異化放中間（例:「30年老師傅」「24小時」）
   - 品牌名放最後
   - 範例: \`花蓮吉安配鑰匙開鎖｜30年老師傅一級棒鎖印行\`

2. **<meta name="description">**：
   - 120-155 字元
   - 包含：地區 + 核心服務 + 差異化 + call to action
   - 自然融入至少 2 個在地關鍵字
...
4. **<link rel="canonical">**：
   ...
   （先放佔位符 {{CANONICAL_URL}},部署時系統會替換成實際網址）

5. **Open Graph 五件套（全部必須有）**：
   \`<meta property="og:type" content="website">\`
   \`<meta property="og:title" content="{同 title}">\`
   \`<meta property="og:description" content="{同 description}">\`
   \`<meta property="og:image" content="{hero 圖片的絕對 URL}">\`
   \`<meta property="og:url" content="{{CANONICAL_URL}}">\`
   \`<meta property="og:locale" content="zh_TW">\`
   \`<meta property="og:site_name" content="{店名}">\`

6. **Twitter Card**：
   \`<meta name="twitter:card" content="summary_large_image">\`
   \`<meta name="twitter:title" content="{同 title}">\`
   \`<meta name="twitter:description" content="{同 description}">\`
   \`<meta name="twitter:image" content="{hero 圖片絕對 URL}">\`

7. **主題色**：
   \`<meta name="theme-color" content="{你選的 primary color hex}">\`

═══════════════════════════════════════════════
【A3. LocalBusiness Schema — 完整版（必須有）】
═══════════════════════════════════════════════

在 <head> 或 </body> 前放一段 \`<script type="application/ld+json">\`,完整範例：

{
  "@context": "https://schema.org",
  "@type": ["LocalBusiness", "{行業對應子類型}"],
  "@id": "{{CANONICAL_URL}}#business",
  "name": "{店名}",
  "description": "{同 meta description}",
  "url": "{{CANONICAL_URL}}",
  "telephone": "{電話,+886 格式}",
  "image": ["{hero_image_url}", "{product1_image_url}"],
  "logo": "{hero 或 logo 圖 URL}",
  "priceRange": "$$",
  "address": {
    "@type": "PostalAddress",
    "streetAddress": "{路段門牌}",
    "addressLocality": "{鄉鎮市區}",
    "addressRegion": "{縣市}",
    "postalCode": "{郵遞區號}",
    "addressCountry": "TW"
  },
  "hasMap": "{Google Maps 連結}",
  "sameAs": ["{Facebook URL}", "{Google Maps URL}"]
}

**行業 → @type 對照表**（從 user message 的產業類別判斷）：
  餐飲 → Restaurant / 咖啡飲料 → CafeOrCoffeeShop
  美容美髮 → BeautySalon 或 HairSalon / 鎖匠 → Locksmith
  汽車保養 → AutoRepair / 零售 → Store / 住宿 → LodgingBusiness
  診所 → MedicalBusiness / 牙醫 → Dentist / 健身 → ExerciseGym
  補教 → EducationalOrganization / 房仲 → RealEstateAgent
  律師會計 → LegalService 或 AccountingService
  其他 → ProfessionalService

**priceRange 規則**: $ 平價 / $$ 中價 / $$$ 高單價

**address 拆解**：請從提供的地址字串自動拆出 streetAddress / addressLocality / addressRegion / postalCode。如果無法判斷郵遞區號就省略。

═══════════════════════════════════════════════
【A4. FAQPage Schema — 有 FAQ 區塊就必須有】
═══════════════════════════════════════════════

如果你的頁面有 FAQ 區塊,**必須同時**輸出 FAQPage JSON-LD,且內容與 HTML 上的 FAQ **完全一致**（Google 會驗證兩邊對不上就不給 rich snippet）：
...
**共同規則**
- H1 結構**必須**包含**在地關鍵字 + 店名**（這是 SEO 最重要的元素）：
    <h1>
        <span class="hero-location">{地區}{核心服務}</span>
        <span class="hero-brand">{店名}</span>
    </h1>
    <p class="hero-tagline">{一句有態度的 statement}</p>
  範例：H1 = 「花蓮吉安鎖匠 一級棒鎖印行」,tagline = 「每一把鑰匙 都是一份承諾」
- tagline 另外放在 <p> 裡,不放在 H1 裡
- 禁止「歡迎光臨 OOO」、「OOO — 您的最佳選擇」這種樣板式文案
- **整頁只能有 1 個 H1**

**Hero 區塊必須有 CTA 按鈕**：
    <div class="hero-cta">
        <a href="tel:+886-X-XXXX-XXXX">{立即來電 電話號碼}</a>
        <a href="#contact">{查看營業時間}</a>
    </div>

**H1 標題階層（嚴格遵守）**：
| H1 | 店名 + 核心在地關鍵字 | 整頁**只能 1 個** |
| H2 | 區塊標題（專業服務、關於我們、常見問題、聯絡我們） | 每區一個 |
| H3 | 服務小項 | 多個 |
| H4 | 服務細項 | 多個 |
- H 階層不可跳級（H2 後面不能直接接 H4）
...
1. **電話必須能一鍵撥打**:
   ❌ \`<p>03-8569085</p>\`
   ✅ \`<a href="tel:+886-3-8569085" aria-label="撥打電話 03-8569085">03-8569085</a>\`
   - 頁面上所有出現電話號碼的地方（包括 Hero meta、聯絡區、footer）都要用 \`<a href="tel:...">\`

2. **地址必須能一鍵開地圖**:
   ❌ \`<p>花蓮縣吉安鄉建國路一段144之2號</p>\`
   ✅ \`<a href="{Google Maps URL}" target="_blank" rel="noopener noreferrer" aria-label="在 Google 地圖開啟店家位置">花蓮縣吉安鄉建國路一段144之2號</a>\`
   - 如果 user message 有提供 Google Maps 連結,就用那個 URL
   - 沒有的話用 \`https://maps.google.com/?q={地址 URL encoded}\`

3. **所有外部連結**都要加 \`target="_blank" rel="noopener noreferrer"\`
...
5. **Hero 文字入場**:用 CSS @keyframes 針對 .hero-location 和 .hero-brand **整塊**做淡入（opacity + translateY）
   ⛔ **絕對不可**把 H1 拆成一堆單字 <span> 做字元逐一淡入 — Google 爬蟲讀到的會是斷字,嚴重傷害 SEO。
   如果真的想做字元逐一淡入效果,只能放在 .hero-tagline（非 H1）上,且不要超過 20 個 <span>。
...
- 請依據每張圖的 slotId 放到語意對應的區塊:
  ...
  - \`extra\` → 任何適合的補充位置
- 每一個 \`<img>\` 標籤**必須**加上對應的 \`data-5888-slot="<slotId>"\` 屬性(例如 \`<img src="..." data-5888-slot="hero" alt="...">\`)。

**圖片 SEO 規則（嚴格遵守）**：

1. **alt 格式**: \`{地區} {服務/商品} {附加描述}\`
   - ✅ \`花蓮吉安一級棒鎖印行建國路店面外觀\`
   - ✅ \`花蓮配汽車晶片鑰匙專業服務\`
   - ❌ \`店面外觀\` / ❌ \`image\` / ❌ \`hero.jpg\` / ❌ 空 alt
   - alt 不可超過 125 字元,不可關鍵字堆砌

2. **載入優先級**:
   - Hero / 首屏圖片: \`loading="eager" fetchpriority="high" decoding="async"\`
   - 非首屏圖片: \`loading="lazy" fetchpriority="auto"\`
   ⛔ Hero 用 loading="lazy" 會嚴重拖累 LCP 分數

3. **尺寸**：所有 \`<img>\` 都**必須**有 \`width\` 和 \`height\` 屬性（避免 CLS 版面跳動）
   - Hero: \`width="1600" height="1067"\`（或實際比例）
   - 內容圖: \`width="800" height="600"\`（或實際比例）

═══════════════════════════════════════════════
【圖片 alt 生成邏輯】
═══════════════════════════════════════════════

依 slotId 自動生成有 SEO 價值的 alt:
  hero     → \`{地區}{店名}{街道名}店面外觀\`
  product1 → \`{地區}{主服務}專業{產品描述}\`
  product2 → \`{地區}{次要服務}{產品描述}\`
  interior → \`{地區}{店名}店內環境\`
  team     → \`{店名}團隊專業{產業類別}師傅\`
  detail   → \`{地區}{工藝/產品}細節特寫\`
...
使用範例:
    <img src="https://..." data-5888-slot="hero" alt="花蓮吉安一級棒鎖印行店面外觀" loading="eager" fetchpriority="high" width="1600" height="1067">
